...
class UIElement:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # === POSITION GROUP ===
        self.parent_anchor = position_dict.get("parent_anchor") or position_dict.get(
            "anchor"
        )  # Backwards compat
        self.self_anchor = position_dict.get("self_anchor") or position_dict.get(
            "align", "top_left"
        )
        ...
        if margin_sides:
            ...
            self.margin_right = (
                margin_sides[1] if len(margin_sides) > 1 else margin_sides[0]
            )
            self.margin_bottom = (
                margin_sides[2] if len(margin_sides) > 2 else margin_sides[0]
            )
            self.margin_left = (
                margin_sides[3] if len(margin_sides) > 3 else margin_sides[1]
            )
        else:
        ...
        if isinstance(text_padding, (list, tuple)):
            ...
            self.text_padding_right = (
                text_padding[1] if len(text_padding) > 1 else text_padding[0]
            )
        else:
        ...
        self.border_color = self._parse_color(
            graphic_dict.get("border_color", [255, 255, 255])
        )
    ...
    def _parse_color(self, color) -> Optional[Union[Tuple[int, ...], GradientColor]]:
        ...
        if isinstance(color, dict) and color.get("type") == "gradient":
            return GradientColor(
                colors=[
                    tuple(c) for c in color.get("colors", [[0, 0, 0], [255, 255, 255]])
                ],
    ...
    def _fill_gradient(
        self, surface: pygame.Surface, gradient: GradientColor, rect: pygame.Rect = None
    ):
    ...
    def _fill_color(
        ...
        color: Union[Tuple[int, ...], GradientColor],
    ...
    def _load_image(self):
        ...
        if self.image_path not in self._draw_manager_ref.images:
            ...
            self._draw_manager_ref.load_image(
                self.image_path, full_path, scale=self.image_scale
            )
    ...
    def _get_text_position(
        self, text_surf: pygame.Surface, container_rect: pygame.Rect
    ) -> pygame.Rect:
        """
        ...
        if self.text_align == "left":
            return text_surf.get_rect(
                midleft=(self.text_padding_left, container_rect.height // 2)
            )
        elif self.text_align == "right":
            return text_surf.get_rect(
                midright=(
                    container_rect.width - self.text_padding_right,
                    container_rect.height // 2,
                )
            )
        else:  # center (default)
            return text_surf.get_rect(
                center=(container_rect.width // 2, container_rect.height // 2)
            )
    ...
    def start_slide_in(
        self, offset: Tuple[int, int], duration: float, delay: float = 0
    ):
        """
